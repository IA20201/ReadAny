...
export async function getFontsDir(): Promise<string> {
  ...
  const fontsDir = await platform.joinPath(appData, FONTS_DIR);
  try {
...
export const useFontStore = create<FontState>((set, get) => ({
  ...
  addFont: (font) => {
    set((state) => {
      ...
      debouncedSave("custom-fonts", { fonts: newFonts });
...
loadFromFS<{ fonts: CustomFont[] }>("custom-fonts").then((data) => {
  if (data?.fonts) {
    useFontStore.setState({ fonts: data.fonts, _hasHydrated: true });
  } else {
