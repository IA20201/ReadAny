...
import { PlusIcon, Trash2Icon, FileTextIcon, LinkIcon, GlobeIcon } from "@/components/ui/Icon";
...
export default function FontSettingsScreen() {
  ...
  const handleConfirmImport = useCallback(async () => {
    ...
    try {
      ...
      addFont(font);
      Alert.alert(t("fonts.imported", "导入成功"), t("fonts.importedDesc", "字体 \"{{name}}\" 已导入", { name: fontNameInput.trim() }));
    } catch (err) {
  ...
  const handleImportRemote = useCallback(async () => {
    ...
    try {
      ...
      addFont(font);
      Alert.alert(t("fonts.imported", "导入成功"), t("fonts.importedDesc", "字体 \"{{name}}\" 已导入", { name: remoteFontName.trim() }));
    } catch (err) {
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent}>
        ...
        {fonts.length === 0 ? (
          <View style={s.emptyState}>
            <FileTextIcon size={48} color={colors.mutedForeground} />
                ...
                <View style={s.fontHeader}>
                  <View style={s.fontInfo}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                      ...
                      {font.source === "remote" && (
                        <View style={[s.remoteBadge, { backgroundColor: colors.primary + "15" }]}>
        ...
        >
          <View style={[s.modalContent, { backgroundColor: colors.card }]}>
            ...
            <View style={s.modalButtons}>
              ...
              <TouchableOpacity
                style={[s.modalBtn, s.modalBtnConfirm, { backgroundColor: colors.primary }]}
